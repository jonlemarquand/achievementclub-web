...
const AchievementCard = ({name, completed, type, date, time, score, text, percentage, link, image}) => {
    return (
        <Link href={link} className="achievement-card">
            {completed && <div className="achievement-card__background"></div>}
            <img className={`achievement-card__img ${completed ? "completed" : ''}`} alt={name} src={image}/>
            ...
            <div className="achievement-card__score">
                <p className={`achievement-card__achievement ${completed ? "completed" : ''}`}>{score}G</p>
